...
public:
	...
	ASSETSTOOLS_API bool ReadBundleFile(void *data, int dataLen, int *filePos, AssetTypeTemplateField *pBaseField);
	ASSETSTOOLS_API void ReadBundleFile(void *data, int dataLen, int *filePos, int assetsVersion);
	ASSETSTOOLS_API void FlushChanges();
	ASSETSTOOLS_API int GetFileSize();
	ASSETSTOOLS_API bool WriteBundleFile(void *buffer, int bufferLen, int *size);
